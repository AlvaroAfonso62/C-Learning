...
void print_cards(char *message, char *cards, int user_pick){
  ...
  printf("\n\t*** %s ***\n", message);
  printf("    \t._.\t._.\t._.\n");
  printf("Cards:\t|%c|\t|%c|\t|%c|\t", cards[0], cards[1], cards[2]);
  if(user_pick == -1)
    printf(" 1 \t 2 \t 3\n");
  else{
...
int take_wager(int available_credits, int previous_wager){
  ...
  scanf("%d", &wager);
  if(wager < 1){
    printf("Nice try, but you must wager a possitive number!\n");
    return -1;
  }
...
int dealer_no_match(){
  int i, j, number[16], wager = -1, match = -1;
...
int find_the_ace(){
  ...
  printf("********** Find the Ace **********\n");
  printf("In this game, you can wager up to all of your credits.\n");
  printf("Theree cards will be dealt out, two queens and one ace.\n");
